...
#ifdef BCMSPI
	/* Flushing rx-fifo.  Not sure if they are because of extra clocks seen
	 * after CS is deasserted.
	 */
	while ((SPIPCI_RREG(osh, &regs->spih_stat) & SPIH_RFEMPTY) == 0)
		sd_err(("Flushed fifo values = 0x%x\n", SPIPCI_RREG(osh, &regs->spih_data)));
#endif /* BCMSPI */
